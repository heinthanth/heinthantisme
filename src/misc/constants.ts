import { absURL } from "./utils";
import siteURL from "./siteURL.mjs";
import cover from "../../assets/pics/cover.png";

export const siteName = "Hein Thant's Space";
export const siteShortName = "Hein's Space";
export const siteDescription = "Just Hein Thant's Space!"
export const description =
  "Just a CS student, Full-Stack web developer, Jr. Flutter developer, Jr. DevOps, Linux and Open-Source lover, and compiler nerd, and also, stupid EDM producer!";
// prettier-ignore
export const searchWords =
  ["Hein Thant Maung Maung", "HIIIiN", "Hein Thanth", "heinthanth",
  "ဟိန်းသန့် မောင်မောင်", "ဟိန်းဟိန်း", "ဟိန်းသန့်"];

export const contactEmail = `hello@${new URL(siteURL).hostname}`;
export const coverLocation = { url: absURL(cover.toString()) };
export const creatorFirstName = "Hein Thant";
export const creatorLastName = "Maung Maung";
export const creatorName = `${creatorFirstName} ${creatorLastName}`
export const creatorUsername = "heinthanth";

// prettier-ignore
export const socialLinks = {
  facebook: `https://facebook.com/${creatorUsername}`,
  instagram: `https://instagram.com/${creatorUsername}`,
  twitter: `https://twitter.com/${creatorUsername}`,
  linkedin: `https://linkedin.com/in/${creatorUsername}`,
  github: `https://github.com/${creatorUsername}`,
  soundcloud: `https://soundcloud.com/${creatorUsername}` };
